...
void AT_MQTT_Event(W61_Object_t *Obj, const uint8_t *rxbuf, int32_t rxbuf_len)
{
  char *ptr = (char *)rxbuf;
  char *str_token;
  W61_CbParamMqttData_t cb_param_mqtt_data;
              ...
              sizeof(W61_MQTT_EVT_SUBSCRIPTION_RECEIVED_KEYWORD) - 1) == 0)
  {
    ...
    /* - Unused - link_id = ParseNumber(str_token, NULL); */
    ...
    cb_param_mqtt_data.topic_length = ParseNumber(str_token, NULL);
    ...
    cb_param_mqtt_data.message_length = ParseNumber(str_token, NULL);
                                           ...
                                           uint8_t *CaCertificate)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE,
             "AT+MQTTUSERCFG=0,%ld,\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"\r\n",
                                       ...
                                       uint32_t WillRetain)
{
  ...
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE,
             "AT+MQTTCONNCFG=0,%ld,%ld,\"%s\",\"%s\",%ld,%ld\r\n",
                                       ...
                                       uint32_t *WillRetain)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    {
      cmp = sscanf((const char *)Obj->CmdResp, "+MQTTCONNCFG:%ld,%ld,\"%[^\"]\",\"%[^\"]\",%ld,%ld\r\n",
...
W61_Status_t W61_MQTT_GetSNI(W61_Object_t *Obj, uint8_t *SNI)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    if (ret == W61_STATUS_OK)
    {
      cmp = sscanf((const char *)Obj->CmdResp, "+MQTTSNI:%ld,\"%[^\"]\"\r\n", &link_id, SNI);
      if (cmp < 2)
      {
        ret = W61_STATUS_ERROR;
      }
...
W61_Status_t W61_MQTT_Connect(W61_Object_t *Obj, uint8_t *Host, uint16_t Port)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+MQTTCONN=0,\"%s\",%d,%d\r\n",
             Host, Port, reconnect);
    ret = AT_SetExecute(Obj, Obj->CmdResp, W61_MQTT_CONNECT_TIMEOUT);
    ATunlock(Obj);
  }
  else
  {
                                          ...
                                          uint32_t *Scheme, uint32_t *State)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    if (ret == W61_STATUS_OK)
    {
      cmp = sscanf((const char *)Obj->CmdResp, "+MQTTCONN:%ld,%ld,%ld,\"%[^\"]\",%ld,%ld\r\n",
...
W61_Status_t W61_MQTT_Subscribe(W61_Object_t *Obj, uint8_t *Topic)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+MQTTSUB=0,\"%s\",%ld\r\n", Topic, qos);
    ret = AT_SetExecute(Obj, Obj->CmdResp, W61_NET_TIMEOUT);
    ATunlock(Obj);
  }
  else
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    if (ATsend(Obj, Obj->CmdResp, strlen((char *)Obj->CmdResp), Obj->NcpTimeout) > 0)
    {
      do
      {
        ...
        recv_len = ATrecv(Obj, Obj->CmdResp, AT_RSP_SIZE, 5000);
        if (recv_len > 0)
        {
          Obj->CmdResp[recv_len] = 0;
    ...
    if (ATsend(Obj, Obj->CmdResp, strlen((char *)Obj->CmdResp), Obj->NcpTimeout) > 0)
    {
      ...
      if (recv_len > 0)
      {
        ...
        if (strncmp((char *) Obj->CmdResp, "+MQTTUNSUB:NO_UNSUBSCRIBE", sizeof("+MQTTUNSUB:NO_UNSUBSCRIBE") - 1) == 0)
        {
          ...
          (void)ATrecv(Obj, Obj->CmdResp, AT_RSP_SIZE, 5000);
...
W61_Status_t W61_MQTT_Publish(W61_Object_t *Obj, uint8_t *Topic, uint8_t *Message, uint32_t Message_len)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+MQTTPUBRAW=0,\"%s\",%ld,%ld,%ld\r\n", Topic,
             Message_len, qos, retain);
    ret = AT_SetExecute(Obj, Obj->CmdResp, W61_NET_TIMEOUT);
    if (ret == W61_STATUS_OK)
    {
      Obj->fops.IO_Delay(10);
      ret = AT_RequestSendData(Obj, Message, Message_len, W61_NET_TIMEOUT, true);
    }
    ATunlock(Obj);
  }
  else
