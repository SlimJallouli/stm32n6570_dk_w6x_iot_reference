...
W61_Status_t W61_RegisterBusIO(W61_Object_t *Obj, IO_Init_Func IO_Init,
...
W61_Status_t W61_Init(W61_Object_t *Obj)
{
  ...
  if (Obj->CmdResp == NULL)
  {
    LogError("Error: Unable to allocate CmdResp");
    return ret;
  }
  ...
  {
    LogError("Error: Unable to allocate EventsBuf");
    goto __err1;
  }

  Obj->ATD_RecvBuf = pvPortCalloc(SPI_XFER_MTU_BYTES, 1);
  if (Obj->ATD_RecvBuf == NULL)
  {
    LogError("Error: Unable to allocate ATD_RecvBuf");
    goto __err2;
  }
  ...
  Obj->ATD_Resp_xMessageBuffer = xMessageBufferCreate(W61_ATD_CMDRSP_XBUF_SIZE);

  if (Obj->ATD_Resp_xMessageBuffer == NULL)
  {
    LogError("Error: Unable to create Obj->ATD_Resp_xMessageBuffer");
    goto __err3;
  }

  /* Create a Event x-buffer that can hold W61_ATD_QUEUE_EVENT_SIZE bytes */
  Obj->ATD_Evt_xMessageBuffer = xMessageBufferCreate(W61_ATD_EVENTS_XBUF_SIZE);

  if (Obj->ATD_Evt_xMessageBuffer == NULL)
  {
    LogError("Error: Unable to create Obj->ATD_Evt_xMessageBuffer");
    goto __err3;
  }

  if (Obj->fops.IO_Init == NULL)
  {
    LogError("Error: IO_Init callback not defined");
    goto __err3;
  }

  ret = (W61_Status_t) Obj->fops.IO_Init();

  xReturned = xTaskCreate(ATD_RxPooling_task,
                          (char *)"AtRxParser",
  ...
  if (xReturned != pdPASS)
  {
    LogError("Error: xTaskCreate failed to create");
    goto __err3;
  }

  xReturned = xTaskCreate(ATD_EventsPooling_task,
                          (char *)"AtEvt",
  ...
  {
    LogError("Error: Wait ready failed");
    goto __err3;
  }

#if (W61_NCP_STORE_MODE == 0)
  /* Disable the store mode */
  ret = W61_SetStoreMode(Obj, 0);
  if (ret != W61_STATUS_OK)
  {
    LogError("Error: Disable store more failed");
    goto __err3;
  }
...
W61_Status_t W61_WaitReady(W61_Object_t *Obj)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    (void)ATrecv(Obj, Obj->CmdResp, AT_RSP_SIZE, 10000);
    ATunlock(Obj);
  }
  else
  {
    LogError("Error: IO Busy");
    ret = W61_STATUS_BUSY;
    goto __err;
  }
  ...
  {
    LogError("Error: No received response from NCP");
    goto __err;
  }
...
W61_Status_t W61_ResetModule(W61_Object_t *Obj)
{
...
#if (W61_NCP_STORE_MODE == 1) /* Not supported to reset the module without reinit all modules */
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+RST\r\n");
    ret = AT_SetExecute(Obj, Obj->CmdResp, 2000);
    ATunlock(Obj);
  }
  else
  {
  ...
  if (ret != W61_STATUS_OK)
  {
    LogError("Error: Reset module failed");
    goto __err;
  }
...
W61_Status_t W61_ResetToFactoryDefault(W61_Object_t *Obj)
{
  ...
  if (ret != W61_STATUS_OK)
  {
    LogError("Error: Restore factory default failed");
    goto __err;
  }
...
W61_Status_t W61_GetNcpHeapState(W61_Object_t *Obj, uint32_t *RemainingHeap)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    if (ret == W61_STATUS_OK)
    {
      if (strncmp((char *)Obj->CmdResp, "+SYSRAM:", sizeof("+SYSRAM:") - 1) == 0)
      {
        *RemainingHeap = ParseNumber((char *)Obj->CmdResp + strlen("+SYSRAM:"), NULL);
      }
    }
    ATunlock(Obj);
  }
...
W61_Status_t W61_GetStoreMode(W61_Object_t *Obj, uint32_t *mode)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    if (ret == W61_STATUS_OK)
    {
      if (sscanf((char *)Obj->CmdResp, "+SYSSTORE:%ld", mode) != 1)
      {
...
W61_Status_t W61_SetStoreMode(W61_Object_t *Obj, uint32_t mode)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+SYSSTORE=%ld\r\n", mode);
    ret = AT_SetExecute(Obj, Obj->CmdResp, Obj->NcpTimeout);
    ATunlock(Obj);
  }
  else
...
W61_Status_t W61_ReadEFuse(W61_Object_t *Obj, uint32_t addr, uint32_t nbytes, uint8_t *data)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+EFUSE-R=%d,\"0x%03x\",1\r\n",
             (int) nbytes, (int) addr);
    ret = AT_Query(Obj, Obj->CmdResp, Obj->CmdResp, Obj->NcpTimeout);
    if (ret == W61_STATUS_OK)
    {
      if (sscanf((char *)Obj->CmdResp, "+EFUSE-R:%ld,", &size) != 1)
      {
        LogError("%s", W61_Parsing_Error_str);
        ret = W61_STATUS_ERROR;
      }
      ...
      {
        str_token = strstr((char *)Obj->CmdResp, ",") + 1;
        memcpy(data, (void *)str_token, size);
        ret = W61_STATUS_OK;
      }
    }
...
W61_Status_t W61_FS_WriteFile(W61_Object_t *Obj, char *filename, uint32_t offset, uint8_t *data, uint32_t len)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+FS=0,2,\"%s\",%ld,%ld\r\n", filename, offset, len);
    ret = AT_SetExecute(Obj, Obj->CmdResp, W61_SYS_TIMEOUT);
    if (ret == W61_STATUS_OK)
    {
      /* Send the chunk of data */
      ret = AT_RequestSendData(Obj, data, len, W61_SYS_TIMEOUT, true);
    }
...
W61_Status_t W61_FS_ReadFile(W61_Object_t *Obj, char *filename, uint32_t offset, uint8_t *data, uint32_t len)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+FS=0,3,\"%s\",%ld,%ld\r\n", filename, offset, len);

    if (ATsend(Obj, Obj->CmdResp, strlen((char *)Obj->CmdResp), Obj->NcpTimeout) > 0)
    {
      /* Wait the response '+FS:READ,' */
      recv_len = ATrecv(Obj, Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, W61_SYS_TIMEOUT);
      if (recv_len > 0)
      {
        /* Get the total length of the data */
        if ((sscanf((char *)Obj->CmdResp, "+FS:READ,%ld,", &total_len) == 1) && (total_len == len))
        {
    ...
    {
      recv_len = ATrecv(Obj, Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, Obj->NcpTimeout);
      if (recv_len > 2)
      {
        Obj->CmdResp[recv_len] = '\0'; /* Null terminate the response */
        ret = AT_ParseOkErr((char *)&Obj->CmdResp[2]); /* Skip the CRLF after last data */
      }
    }
...
W61_Status_t W61_FS_GetSizeFile(W61_Object_t *Obj, char *filename, uint32_t *size)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    ret = AT_Query(Obj, Obj->CmdResp, Obj->CmdResp, W61_SYS_TIMEOUT);
    if (ret == W61_STATUS_OK)
    {
      if (sscanf((char *)Obj->CmdResp, "+FS:SIZE,%ld", size) != 1)
      {
        ret = W61_STATUS_ERROR;
      }
    }
...
W61_Status_t W61_GetModuleInfo(W61_Object_t *Obj)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    char dummy_buffer[100]; /* To decide if adding those versions to the Obj */
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+GMR\r\n");
    if (ATsend(Obj, Obj->CmdResp, strlen((char *)Obj->CmdResp), Obj->NcpTimeout) > 0)
    {
      W61_VersionInfo_t version_table[] =
      {
        {(char *)Obj->ModuleInfo.AT_Version, "AT version:", 32, 100},
        {(char *)Obj->ModuleInfo.MAC_Version, "component_version_macsw_", 32, 100},
        {dummy_buffer, "component_version_bthost_", 32, 100},
        {dummy_buffer, "component_version_btblecontroller_", 32, 100},
        {dummy_buffer, "component_version_btprofile_", 32, 100},
        {dummy_buffer, "component_version_phyrf_", 32, 100},
        {(char *)Obj->ModuleInfo.SDK_Version, "component_version_sdk_", 32, 100},
        {dummy_buffer, "compile time:", 32, 100}
      };

      for (int32_t i = 0; i < sizeof(version_table) / sizeof(version_table[0]); i++)
      {
        recv_len = ATrecv(Obj, Obj->CmdResp, AT_RSP_SIZE, version_table[i].timeout);
        if (recv_len > 0)
        {
          ...
          if (strncmp(version_table[i].prefix, "AT version:", sizeof("AT version:") - 1) == 0)
          {
            ...
            if (date != NULL)
            {
              ...
              *date = 0;
              strncpy((char *)Obj->ModuleInfo.Build_Date, date + 1, sizeof(Obj->ModuleInfo.Build_Date));
              /* Remove last ')' character */
              Obj->ModuleInfo.Build_Date[strlen((char *)Obj->ModuleInfo.Build_Date) - 1] = 0;
            }
          }
        }
        else
      ...
      /* Dummy receive */
      recv_len = ATrecv(Obj, Obj->CmdResp, AT_RSP_SIZE, 500);
      if (recv_len > 0)
      {
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    if (ret == W61_STATUS_OK)
    {
      ...
      {
        Obj->ModuleInfo.BatteryVoltage = ParseNumber((char *)Obj->CmdResp + strlen("+VBAT:"), NULL);
      }
    }
    ATunlock(Obj);
  }
  ...
  for (uint32_t i = 0; i < sizeof(trim_table) / sizeof(trim_table[0]); i++)
  {
    ...
    if (ret != W61_STATUS_OK)
    {
      ...
      {
        LogError("Error: Unable to read %s", trim_table[i].desc);
      }
      goto _err;
    }
    ...
    if ((data[0] >> trim_table[i].en_offset) & 0x1) /* Check if trim is enabled */
    {
      ...
      {
        /*
        LogInfo("%s: %d,%d,%d,%d,%d", trim_table[i].desc, trim_value & 0x1f, (trim_value >> 5) & 0x1f,
                (trim_value >> 10) & 0x1f, (trim_value >> 15) & 0x1f, (trim_value >> 20) & 0x1f);
        */
      ...
      else if ((trim_table[i].type == 0) || (trim_table[i].type == 1)) /* Wi-Fi */
      {
        ...
        /*
        LogInfo("%s: %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", trim_table[i].desc,
                pwr_offset[0], pwr_offset[1], pwr_offset[2], pwr_offset[3], pwr_offset[4],
                pwr_offset[5], pwr_offset[6], pwr_offset[7], pwr_offset[8], pwr_offset[9],
                pwr_offset[10], pwr_offset[11], pwr_offset[12], pwr_offset[13]);
        */
  ...
  if (ret != W61_STATUS_OK)
  {
    if (ret == W61_STATUS_ERROR)
    {
      LogError("Error: Unable to read Part number");
    }
    goto _err;
  }
  ...
  if (ret != W61_STATUS_OK)
  {
    if (ret == W61_STATUS_ERROR)
    {
      LogError("Error: Unable to read BOM ID & Manufacturing Year/Week");
    }
    goto _err;
  }
  ...
  for (int32_t i = 0; i < sizeof(efuse_mac_table) / sizeof(efuse_mac_table[0]); i++)
  {
    ...
    if (ret != W61_STATUS_OK)
    {
      if (ret == W61_STATUS_ERROR)
      {
        LogError("Error: Unable to read %s", efuse_mac_table[i].desc);
      }
      goto _err;
    }
    ...
    {
      /* LogInfo("%s is not defined in EFuse", efuse_mac_table[i].desc); */
    }
  }
  ...
  if (ret != W61_STATUS_OK)
  {
    if (ret == W61_STATUS_ERROR)
    {
      LogError("Error: Unable to read Anti-rollback enable");
    }
    goto _err;
  }
  ...
  if ((data[0] >> 12) & 0x1) /* Anti-rollback enabled */
  {
    ...
    for (int32_t i = 0; i < sizeof(efuse_antirollback_table) / sizeof(efuse_antirollback_table[0]); i++)
    {
      ...
      if (ret != W61_STATUS_OK)
      {
        if (ret == W61_STATUS_ERROR)
        {
          LogError("Error: Unable to read Anti-rollback %s", efuse_antirollback_table[i].desc);
        }
        goto _err;
      }
      ...
      {
        *efuse_antirollback_table[i].data += 32 - __builtin_clz(data[j] | data[j + half_index]);
      }
    }
  }
...
W61_Status_t W61_OTA_starts(W61_Object_t *Obj, uint32_t enable)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+OTASTART=%ld\r\n", enable);
    ret = AT_SetExecute(Obj, Obj->CmdResp, W61_SYS_TIMEOUT);
    ATunlock(Obj);
  }
  else
...
W61_Status_t W61_OTA_Send(W61_Object_t *Obj, uint8_t *buff, uint32_t len)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+OTASEND=%ld\r\n", len);
    ret = AT_SetExecute(Obj, Obj->CmdResp, W61_SYS_TIMEOUT);
    if (ret == W61_STATUS_OK)
    {
      Obj->fops.IO_Delay(OTA_DATA_SEND_DELAY);
      ret = AT_RequestSendData(Obj, buff, len, W61_SYS_TIMEOUT, true);
    }
    ATunlock(Obj);
  }
  else
...
W61_Status_t W61_LowPowerConfig(W61_Object_t *Obj, uint32_t WakeUpPinIn, uint32_t ps_mode)
{
  ...
  Obj->LowPowerCfg.PSAutoEnable = 0;
...
W61_Status_t W61_SetPowerMode(W61_Object_t *Obj, uint32_t ps_mode, uint32_t hbn_level)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    ...
    {
      snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+PWR=%ld,%ld\r\n", ps_mode, hbn_level);
    }
    else
    {
      snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+PWR=%ld\r\n", ps_mode);
    }
    ret = AT_SetExecute(Obj, Obj->CmdResp, Obj->NcpTimeout);
...
W61_Status_t W61_LowPowerEnterUnderLock(W61_Object_t *Obj)
{
  ...
  if (Obj->LowPowerCfg.PSAutoEnable == 1)
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+PWR=2\r\n");
    ret =  AT_SetExecute(Obj, Obj->CmdResp, Obj->NcpTimeout);
  }
...
{
  Obj->LowPowerCfg.PSAutoEnable = enable;
  return W61_STATUS_OK;
}

W61_Status_t W61_GetPowerSaveAuto(W61_Object_t *Obj, uint32_t *enable)
{
  *enable = Obj->LowPowerCfg.PSAutoEnable;
  return W61_STATUS_OK;
}
...
W61_Status_t W61_SetWakeUpPin(W61_Object_t *Obj, uint32_t wakeup_pin)
{
  ...
  if (ATlock(Obj, AT_LOCK_TIMEOUT))
  {
    snprintf((char *)Obj->CmdResp, W61_ATD_CMDRSP_STRING_SIZE, "AT+SLWKIO=%ld,0\r\n", wakeup_pin);
    ret = AT_SetExecute(Obj, Obj->CmdResp, Obj->NcpTimeout);
    ATunlock(Obj);
  }
  else
