...
/**
  ...
  *
  * Reference source: examples/stm32_spi_host/QCC743_SPI_HOST/src/spi_port.h
  */
...
#define SPI_PORT_TRACE_ENABLE           1
...
#define spi_log(M, ...) do { LogDebug("[%u][%s +%d] " M "",\
...
#define spi_dbg(M, ...) do { LogDebug("[%u][%s +%d] " M "",\
                             (xPortIsInsideInterrupt())?(xTaskGetTickCountFromISR()):(xTaskGetTickCount()),\
                             SHORT_FILE, __LINE__,\
                             ##__VA_ARGS__);\
                           } while(0)
...
#define spi_err(M, ...) do { LogError("[%u][%s +%d] " M "",\
                             (xPortIsInsideInterrupt())?(xTaskGetTickCountFromISR()):(xTaskGetTickCount()),\
                             SHORT_FILE, __LINE__,\
                             ##__VA_ARGS__);\
                           } while(0)
...
#if 1
#define spi_trace(e, ...) printf( __VA_ARGS__)
#else
