...
void mem_perf_report(void)
{
  ...
  /* Display the summary */
  LogInfo("### %s", report_title);
  LogInfo("# Max allocated:        %" PRIu32 " bytes", max_alloc);
  LogInfo("# Current leakage:      %" PRIu32 " bytes", current_alloc);
  LogInfo("# Nb Max allocated:     %" PRIu32, iteralloc);
  LogInfo("# Nb Current allocated: %" PRIu32, iteralloc - iterfree);

  LogInfo("%s", separator);
  LogInfo("### Remaining Mem report");
  LogInfo("%-7s%-14s%-6s%-10s", "id", "address", "size", "task");
  LogInfo("%s", separator);
  ...
  for (uint32_t i = 0; i < LEAKAGE_ARRAY; i++)
  {
    if (mem_perf[i].p != 0)
    {
      ...
      LogInfo("#%04" PRIu32 "  %p%8" PRIu32 "  %10s", (uint32_t)mem_perf[i].iter, mem_perf[i].p, mem_perf[i].size,
              mem_perf[i].name);
