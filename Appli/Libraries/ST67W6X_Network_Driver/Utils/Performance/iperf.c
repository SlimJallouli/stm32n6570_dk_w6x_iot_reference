...
typedef struct
{
  ...
  uint32_t auto_enable;   /*!< Low power auto enable */
...
#define IPERF_LOGE(format, ...)                             \
  do {                                                      \
    LogError("[iperf] " format, ##__VA_ARGS__); \
  } while (0)
...
int32_t iperf_start(iperf_cfg_t *cfg)
{
  ...
  {
    s_iperf_ctrl.buffer_len = IPERF_UDP_RX_LEN;
  }
  ...
  {
    IPERF_LOGE("create buffer: not enough memory");
    return IPERF_FAIL;
  }
  ...
  {
    IPERF_LOGE("create task %s failed", IPERF_TRAFFIC_TASK_NAME);
...
static int32_t iperf_show_socket_error_reason(const char *str, int32_t sockfd)
{
  ...
  if (err != 0)
  {
    LogInfo("%s error, error code: %d, reason: %s", str, err, strerror(err));
  }
...
static void iperf_report_task(void *arg)
{
  ...
  volatile double actual_transfer = 0;
  ...
  while (!s_iperf_ctrl.finish)
  {
    ...
    else if ((!s_iperf_ctrl.finish) && (interval_sec != 0) && (elapsed_time % interval_sec == 0))
    {
      /* Calculate the actual bytes transferred */
      actual_transfer = s_iperf_ctrl.actual_len / 1e6;
      /* Calculate the actual bandwidth */
      actual_bandwidth = (actual_transfer * 8) / interval_sec;
      /* Calculate the average bandwidth from the start */
      average = ((average * (elapsed_time - 1) / elapsed_time) + (actual_bandwidth / elapsed_time));

      LogInfo("[%3d] %2d.0-%2d.0 sec  %5.2f MBytes    %5.2f Mbits/sec",
              s_iperf_ctrl.sockfd, start_time, start_time + interval_sec, actual_transfer, actual_bandwidth);
  ...
  if (((s_iperf_ctrl.cfg.flag & IPERF_FLAG_CLIENT) && (elapsed_time >= time)) ||
      ((s_iperf_ctrl.cfg.flag & IPERF_FLAG_SERVER) && (s_iperf_ctrl.cfg.flag & IPERF_FLAG_TCP)))
  {
    ...
    actual_transfer = (double)(s_iperf_ctrl.tot_len / 1e6);
    /* Calculate the average bandwidth from the start */
    average = (double)((actual_transfer * 8) / elapsed_time);

    LogInfo("[%3ld]  0.0-%2ld.0 sec  %5.2f MBytes    %5.2f Mbits/sec",
            s_iperf_ctrl.sockfd, elapsed_time, actual_transfer, average);
  }
...
static iperf_err_t iperf_start_report(void)
{
  ...
  {
    IPERF_LOGE("create task %s failed", IPERF_REPORT_TASK_NAME);
    return IPERF_FAIL;
  }
      ...
      if (type == IPERF_TRANS_TYPE_UDP)
      {
        ...
        if (datagramID >= 0) /* If the datagram ID is negative, it means the end of the test */
        {
          ...
          if (lastTransit != 0)
          {
            ...
            stats.jitter += (((double)deltaTransit / 1e6) - stats.jitter) / (16.0);
          }
          lastTransit = transit;
#endif /* JITTER_RX */
        ...
        else if (s_iperf_ctrl.finish == false)
        {
          ...
          if (actual_recv > (int)(sizeof(UDP_datagram_t) + sizeof(iperf_server_hdr_t)))
          {
            ...
            stats.endTime = ((double)lastPacketTime.sec  - (double)firstPacketTime.sec) + \
                            ((double)lastPacketTime.usec - (double)firstPacketTime.usec) / ((double) 1e6);
            server_hdr->stop_sec     = PP_HTONL((long) stats.endTime);
            server_hdr->stop_usec    = PP_HTONL((long)((stats.endTime - (long)stats.endTime) * 1e6));
            ...
            double total_transfer = s_iperf_ctrl.tot_len / 1e6;
            /* Calculate the average bandwidth from the start */
            double average = (total_transfer * 8) / stats.endTime;

            LogInfo("[%3d] %2d.0-%4.1f sec  %5.2f MBytes    %5.2f Mbits/sec",
                    s_iperf_ctrl.sockfd, 0, stats.endTime, total_transfer, average);

#if JITTER_RX
            LogInfo("[ ID]  Jitter        Lost/Total Datagrams");
            LogInfo("[%3d] %6.3f ms      %4d/%5d (%.2g%%)", s_iperf_ctrl.sockfd,
            ...
            LogInfo("[%3d]                %4ld/%5ld (%3ld%%)", s_iperf_ctrl.sockfd,
                    stats.cntError, (stats.cntDatagrams + stats.cntError),
                    100 * (stats.cntError / (stats.cntDatagrams + stats.cntError)));
#endif /* JITTER_RX */
          }
    ...
    while (count < 2) /* Try to receive the report packet from the server */
    {
      ...
      if (rc > 0)
      {
        ...
        if (rc >= (int)(sizeof(UDP_datagram_t) + sizeof(iperf_server_hdr_t)))
        {
          ...
          LogInfo("[ ID]  Jitter        Lost/Total Datagrams");
          LogInfo("[%3d] %ld.%03ld ms    %4ld/%5ld (%3ld%%)", send_socket,
                  PP_HTONL(server_hdr->jitter1) * 1000, PP_HTONL(server_hdr->jitter2) * 1000,
                  PP_HTONL(server_hdr->error_cnt), PP_HTONL(server_hdr->datagrams),
                  100 * (PP_HTONL(server_hdr->error_cnt) / PP_HTONL(server_hdr->datagrams)));
        }
        return;
      }
      else if (rc < 0)
...
static iperf_err_t iperf_run_tcp_server(void)
{
  ...
  if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6)
  {
    ...
    {
      ret = IPERF_FAIL;
      IPERF_LOGE("Unable to create socket: errno %d", errno);
      goto exit;
    }
    ...
    {
      ret = IPERF_FAIL;
      IPERF_LOGE("Socket unable to bind: errno %d, IPPROTO: %d", errno, AF_INET6);
      goto exit;
    }
    ...
    {
      ret = IPERF_FAIL;
      IPERF_LOGE("Error occurred during listen: errno %d", errno);
      goto exit;
    }
...
#endif /* IPERF_V6 */
  {
    if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV4)
    {
      ...
      {
        ret = IPERF_FAIL;
        IPERF_LOGE("Socket unable to bind: errno %d, IPPROTO: %d", errno, AF_INET);
        goto exit;
      }
  ...
  {
    ret = IPERF_FAIL;
    IPERF_LOGE("Unable to accept connection: errno %d", errno);
    goto exit;
  }
...
static iperf_err_t iperf_run_tcp_client(void)
{
  ...
  if ((s_iperf_ctrl.cfg.flag & IPERF_FLAG_CLIENT) && (s_iperf_ctrl.cfg.flag & IPERF_FLAG_TCP)
      && (s_iperf_ctrl.cfg.flag & IPERF_FLAG_DUAL))
  {
    xTaskCreate(iperf_tcp_dual_server_task, "dual_rx", IPERF_TRAFFIC_TASK_STACK, NULL,
                s_iperf_ctrl.cfg.traffic_task_priority, NULL);
  ...
  if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6)
  {
    ...
    {
      ret = IPERF_FAIL;
      IPERF_LOGE("Socket unable to connect: errno %d", errno);
      goto exit;
    }
...
static iperf_err_t iperf_run_udp_server(void)
{
  ...
  if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6)
  {
    ...
    {
      ret = IPERF_FAIL;
      IPERF_LOGE("Socket unable to bind: errno %d", errno);
      goto exit;
    }
...
void iperf_task_traffic(void *arg)
{
  ...
  W6X_WiFi_GetDTIM(&s_iperf_ctrl.dtim);
  W6X_GetPowerSaveAuto(&s_iperf_ctrl.auto_enable);
  W6X_GetPowerMode(&s_iperf_ctrl.ps_mode);
  ...
  else if ((s_iperf_ctrl.cfg.flag & IPERF_FLAG_CLIENT) && (s_iperf_ctrl.cfg.flag & IPERF_FLAG_UDP))
  {
    ...
    /*char *ip_ptr = */(void)W6X_Net_Inet_ntop(AF_INET, (void *) & (s_iperf_ctrl.cfg.destination_ip4), ipaddr,
                                               INET_ADDRSTRLEN + 1);
    LogInfo("Client connecting to %s, UDP port %d", ipaddr, s_iperf_ctrl.cfg.dport);
    if (s_iperf_ctrl.cfg.num_bytes)
    {
      LogInfo("Sending %.0f Mbytes datagrams", s_iperf_ctrl.cfg.num_bytes / (double)1e6);
    }
  }
  else if ((s_iperf_ctrl.cfg.flag & IPERF_FLAG_CLIENT) && (s_iperf_ctrl.cfg.flag & IPERF_FLAG_TCP))
  {
    ...
    /*char *ip_ptr = */(void)W6X_Net_Inet_ntop(AF_INET, (void *) & (s_iperf_ctrl.cfg.destination_ip4), ipaddr,
                                               INET_ADDRSTRLEN + 1);
    LogInfo("Client connecting to %s, TCP port %d", ipaddr, s_iperf_ctrl.cfg.dport);
  }
  LogInfo("------------------------------------------------------------");
  ...
  W6X_SetPowerSaveAuto(s_iperf_ctrl.auto_enable);
  W6X_SetPowerMode(s_iperf_ctrl.ps_mode);
  W6X_WiFi_SetDTIM(s_iperf_ctrl.dtim);
