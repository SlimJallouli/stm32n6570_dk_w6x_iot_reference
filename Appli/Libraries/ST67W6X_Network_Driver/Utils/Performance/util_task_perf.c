...
void task_perf_report(void)
{
  ...
  if (task_perf.state)
  {
    ...
    LogInfo("### %s CPU Freq %3"PRIu32" Mhz", report_title, SystemCoreClock / 1000000U);

    /* Display the header */
    LogInfo("%-17s%-16s%-14stime (ms)", "thread id", "name", "cycles");
    LogInfo("%s", separator);

    {
      ...
      while ((count < PERF_MAXTHREAD) && task_perf.handle[count] != 0)
      {
        ...
        LogInfo("thread #%" PRIu32 "  %10s   %15s  %10" PRIu32, count, task_perf.name[count],
    ...
    LogInfo("%-38s%15s  %10" PRIu32, "Total", cycle_string64,
            (uint32_t)(total_cycle / (SystemCoreClock / 1000U)));

    LogInfo("### %s end", report_title);
  }
  else
  {
    LogInfo("### %s not started", report_title);
  }
#endif /* TASK_PERF_ENABLE */
}
...
SHELL_CMD_EXPORT_ALIAS(task_perf_shell, task_perf, start or stop [-s] task performance measurement);
...
SHELL_CMD_EXPORT_ALIAS(task_perf_shell_report, task_report, display task performance report);
...
SHELL_CMD_EXPORT_ALIAS(task_alloc_shell_report, ps, display a table of tasks in the system);
