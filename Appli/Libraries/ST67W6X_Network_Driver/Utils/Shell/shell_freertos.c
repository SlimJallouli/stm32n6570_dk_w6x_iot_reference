...
struct _shell_freertos
{
  ...
  /** Stream buffer handle used to store the user input received from the uart rx. */
...
void shell_freertos_init(UART_HandleTypeDef *p_rx_uart, void *xLogQueue)
{
  ...
  p_shell->mb = xMessageBufferCreate(SHELL_FREERTOS_RX_BUFF_SIZE);
  configASSERT(p_shell->mb != NULL);
  ...
  if (status != HAL_OK)
  {
    SHELL_E("failed to setup uart reception, %d\r\n", status);
  }
#endif /* SHELL_ENABLE */
}
...
void shell_freertos_deinit(void)
{
  ...
  if (p_shell->mb != NULL)
  {
    vMessageBufferDelete(p_shell->mb);
    p_shell->mb = NULL;
  }
...
void shell_freertos_rx_task(void *p_params)
{
  ...
  while (1)
  {
    if (1 == xMessageBufferReceive(p_shell->mb, &data, 1, portMAX_DELAY))
    {
      shell_handler(data);
    }
  }
...
static void shell_freertos_on_new_data(uint8_t uart_rxbyte)
{
  ...
  if (xMessageBufferSend(p_shell->mb, &uart_rxbyte, 1, 0) != pdPASS)
  {
    SHELL_E("Failed to send data to the shell task\r\n");
  }
}
...
void shell_freertos_printf(const char *const p_format, ...)
{
  ...
  if (xQueueSendToBack(p_shell->OutputQueue, &newShell, 0) != pdPASS)
  {
    ...
    vPortFree(newShell);
    printf("Shell Log queue full, message dropped\n");
  }
}
