...
void shell_handler(uint8_t data)
{
  ...
  if (data == '\r' || data == '\n')
  {
    ...
    else if (cmd_ret != -255) /* -255: no command */
    {
      SHELL_PRINTF("ERROR\r\n");
    }
#else
...
SHELL_CMD_EXPORT_ALIAS(shell_help, help, display all available commands and the relative help message.);
...
int32_t shell_memtrace(int32_t argc, char **argv)
{
  ...
  /* Get address */
  addr = ParseHexNumber(argv[1], NULL);
  count = ParseHexNumber(argv[2], NULL);
  ...
  {
    /* Write value */
    value = ParseHexNumber(argv[3], NULL);
