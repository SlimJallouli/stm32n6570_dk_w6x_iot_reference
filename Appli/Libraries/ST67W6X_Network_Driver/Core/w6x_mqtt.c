...
W6X_Status_t W6X_MQTT_Init(W6X_MQTT_Data_t *p_mqtt_config)
{
  ...
  p_DrvObj = W61_ObjGet();
  if ((p_DrvObj == NULL) || (p_mqtt_config == NULL))
  {
    return ret;
  }
...
W6X_Status_t W6X_MQTT_Configure(W6X_MQTT_Connect_t *Config)
{
  ...
  uint32_t file_found;
  uint8_t *certificate_lst[3] = {Config->Certificate, Config->PrivateKey, Config->CACertificate};
  ...
  {
    if (certificate_lst[cert_index][0] != '\0')
    {
      ...
      if (file_found == 0)
      {
        ...
        {
          LogError("Writing file %s into the NCP failed", certificate_lst[cert_index]);
          goto _err;
        }
      }
    }
...
static void W6X_mqtt_cb(W61_event_id_t event_id, void *event_args)
{
  W6X_CbParamMqttData_t *cb_param_mqtt_data;
  ...
  switch (event_id)
  {
    ...
    case W61_MQTT_EVT_SUBSCRIPTION_RECEIVED_ID:
      cb_param_mqtt_data = (W6X_CbParamMqttData_t *)event_args;
