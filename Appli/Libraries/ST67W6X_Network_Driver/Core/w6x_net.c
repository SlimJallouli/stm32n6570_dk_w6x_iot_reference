...
typedef struct
{
  ...
  W6X_Socket_Status_t Status;   /*!< Socket status */
  W6X_Protocol_t Protocol;      /*!< Connection type. Parameter is valid only if connection is made as client */
} W6X_Socket_t;
...
typedef struct
{
  ...
  W6X_Tls_Credential_t type;  /*!< Type of the credential */
...
typedef struct
{
  W6X_Socket_t Sockets[W61_NET_MAX_CONNECTIONS + 1];          /*!< Socket context */
  W6X_Connection_t Connection[W61_NET_MAX_CONNECTIONS];       /*!< NCP connections context */
  W6X_Credential_t Credentials[W61_NET_MAX_CONNECTIONS * 3];  /*!< Credentials context */
...
#define RECV_TIMEOUT          5000        /*!< Receive timeout in ms */

#define SO_DEFAULT_TIMEOUT    2000  /*!< Default timeout in ms for SO_RCVTIMEO and SO_SNDTIMEO */

/** Default Net receive buffer size
...
#define NET_RECV_BUFFER_SIZE  (2 * 3 * 1536)
...
/**
  ...
  * @retval Net status
  */
static int32_t W6X_Net_TranslateErrorStatus(uint32_t ret61);
...
W6X_Status_t W6X_Net_Init(void)
{
  ...
  p_net_ctx = pvPortCalloc(sizeof(W6X_NetCtx_t), 1);
  if (p_net_ctx == NULL)
  {
  ...
  ret = TranslateErrorStatus(W61_Net_Init(p_DrvObj));
  ...
  {
    memset(&p_net_ctx->Connection[i], 0, sizeof(W6X_Connection_t));
    ...
    /* Define the buffer length available in NCP for each connection */
    ret = TranslateErrorStatus(W61_Net_SetReceiveBufferLen(p_DrvObj, i, NET_RECV_BUFFER_SIZE));
    if (ret != W6X_STATUS_OK)
    {
    ...
    {
      if (NET_RECV_BUFFER_SIZE != ncp_buf_size_resp)
      {
        LogWarn("SO_RCVBUF was not set as expected, size id %d", ncp_buf_size_resp);
      }
    }
  }
  ...
  {
    memset(&p_net_ctx->Sockets[i], 0, sizeof(W6X_Socket_t));
    /* Set the socket number to an invalid value */
    p_net_ctx->Sockets[i].Number = W61_NET_MAX_CONNECTIONS + 1;
    p_net_ctx->Sockets[i].SoSndTimeo = SO_DEFAULT_TIMEOUT; /* Default value for SO_SNDTIMEO */
    p_net_ctx->Sockets[i].RecvTimeout = SO_DEFAULT_TIMEOUT; /* Default value for SO_RCVTIMEO */
  }

  for (uint8_t i = 0; i < (W61_NET_MAX_CONNECTIONS * 3); i++) /* Initialize the credentials */
  {
    p_net_ctx->Credentials[i].is_valid = 0;
    p_net_ctx->Credentials[i].credential = NULL;
  }
  p_net_ctx->NextSocketToUse = 0;
_err:
  return ret;
}
...
int32_t W6X_Net_Socket(int32_t family, int32_t type, int32_t proto)
{
  ...
  for (int32_t i = 0; i < (W61_NET_MAX_CONNECTIONS + 1); i++) /* Find an available socket */
  {
    ...
    {
      memset(&p_net_ctx->Sockets[socket_to_use], 0, sizeof(W6X_Socket_t)); /* Reset the socket */
      if ((type == SOCK_STREAM) && (proto == TCP_PROTOCOL)) /* TCP */
      {
        p_net_ctx->Sockets[socket_to_use].Protocol = W6X_TCP_PROTOCOL;
      }
      else if ((type == SOCK_DGRAM) && (proto == UDP_PROTOCOL)) /* UDP */
      {
        p_net_ctx->Sockets[socket_to_use].Protocol = W6X_UDP_PROTOCOL;
      }
      else if ((type == SOCK_STREAM) && (proto == SSL_PROTOCOL)) /* SSL */
      {
        p_net_ctx->Sockets[socket_to_use].Protocol = W6X_SSL_PROTOCOL;
      }
      else
      {
      ...
      p_net_ctx->Sockets[socket_to_use].RecvTimeout = RECV_TIMEOUT;
      p_net_ctx->Sockets[socket_to_use].RecvBuffSize = NET_RECV_BUFFER_SIZE;
...
int32_t W6X_Net_Close(int32_t sock)
{
  ...
  memset(&p_net_ctx->Sockets[sock], 0, sizeof(W6X_Socket_t)); /* Reset the socket */
  ...
  if (close_connection == 1) /* Requested to close all pending connections */
  {
    for (int32_t i = 0; i <  W61_NET_MAX_CONNECTIONS + 1; i++)
    {
      ...
      if ((p_net_ctx->Sockets[i].Status != W6X_SOCKET_RESET) && (i != sock) && (p_net_ctx->Sockets[i].Client == 0))
      {
        ...
        {
          LogError("Could not close connection %d", p_net_ctx->Sockets[i].Number);
        }
      }
    }
  }
...
int32_t W6X_Net_Bind(int32_t sock, const struct sockaddr *addr, socklen_t addrlen)
{
  ...
  {
    LogError("Wrong Unexpected socket status: %d status is %d", sock, p_net_ctx->Sockets[sock].Status);
    return ret;
  }
...
int32_t W6X_Net_Connect(int32_t sock, const struct sockaddr *addr, socklen_t addrlen)
{
  ...
  {
    case W6X_TCP_PROTOCOL:
      conn.Type = W61_TCP_CONNECTION;
      break;
    case W6X_UDP_PROTOCOL:
      conn.Type = W61_UDP_CONNECTION;
      break;
    case W6X_SSL_PROTOCOL:
      conn.Type = W61_TCP_SSL_CONNECTION;
      int32_t auth_mode = 0;
      if (p_net_ctx->Sockets[sock].Ca_Cert != NULL)
      {
        auth_mode++;
...
int32_t W6X_Net_Listen(int32_t sock, int32_t backlog)
{
  ...
  {
    case W6X_TCP_PROTOCOL:
      type = W61_TCP_CONNECTION;
      break;
    case W6X_UDP_PROTOCOL:
      type = W61_UDP_CONNECTION;
      break;
    case W6X_SSL_PROTOCOL:
      type = W61_TCP_SSL_CONNECTION;
      break;
    default:
      type = W61_TCP_CONNECTION;
      break;
...
int32_t W6X_Net_Accept(int32_t sock, struct sockaddr *addr, socklen_t *addrlen)
{
  ...
  if (p_net_ctx->Sockets[sock].Protocol == W6X_TCP_PROTOCOL) /* TCP */
  {
    type = SOCK_STREAM;
    proto = 6;
  }
                       ...
                       socklen_t addrlen)
{
  ...
  if (p_net_ctx->Sockets[sock].Protocol != W6X_UDP_PROTOCOL) /* Check if the socket protocol is UDP */
  {
                         ...
                         socklen_t *addrlen)
{
  ...
  if (p_net_ctx->Sockets[sock].Protocol != W6X_UDP_PROTOCOL)
  {
    LogError("Socket protocol is not UDP");
    return -1;
  }
  ...
  if (p_net_ctx->Sockets[sock].Status == W6X_SOCKET_LISTENING)
  {
    while (1)
    {
      ...
      {
        LogError("No data received within %d s", p_net_ctx->Sockets[sock].RecvTimeout / 1000);
        return 0;
      }
    }
    ...
    {
      /* Check if the socket is still connected */
      LogDebug("No Data available to read in connection %d", udp_connection);
    }
  }
  else
  {
    LogDebug("No Data available to read in socket %d", p_net_ctx->Sockets[sock].Number);
    return 0;
  }
...
char *W6X_Net_Inet_ntop(int32_t af, const void *src, char *dst, socklen_t size)
{
  ...
  {
    case AF_INET:
      snprintf(dst, size, "%d.%d.%d.%d", (uint8_t)(src_addr >> 24), (uint8_t)((src_addr >> 16) & 0xff),
...
int32_t W6X_Net_Setsockopt(int32_t sock, int32_t level, int32_t optname, const void *optval, socklen_t optlen)
{
  ...
  else if (level == SOL_TLS && p_net_ctx->Sockets[sock].Protocol == W6X_SSL_PROTOCOL)
  {
    switch (optname)
    {
...
int32_t W6X_Net_Tls_Credential_Add(uint32_t tag, W6X_Tls_Credential_t type, const void *cred, size_t credlen)
{
  if (tag >= W61_NET_MAX_CONNECTIONS * 3)
  {
    LogError("Tag cannot be higher than %d, value is %d", W61_NET_MAX_CONNECTIONS * 3, tag);
    return -1;
  }
  if (p_net_ctx->Credentials[tag].is_valid == 1)
  {
    LogError("Tag %d already in use", tag);
  ...
  uint32_t size = 0;
  if (type == TLS_CREDENTIAL_PSK || type == TLS_CREDENTIAL_PSK_ID)
  {
  ...
  {
    LogError("Writing file %s into the NCP failed", p_net_ctx->Credentials[tag].credential);
    return -1;
  }
  p_net_ctx->Credentials[tag].is_valid = 1;
  return 0;
...
int32_t W6X_Net_Tls_Credential_Delete(uint32_t tag, W6X_Tls_Credential_t type)
{
  if (tag >= W61_NET_MAX_CONNECTIONS * 3)
  {
    return -1;
...
static void W6X_net_cb(W61_event_id_t event_id, void *event_args)
{
  W61_CbParamNetData_t *p_param_net_data = (W61_CbParamNetData_t *) event_args;
  ...
  switch (event_id) /* Check the event ID and call the appropriate callback */
  {
    ...
    case W61_NET_EVT_SOCK_CONNECTED_ID:
      LogDebug(" W6X got NET_EVT_SOCK_CONNECTED_ID for connection %d", p_param_net_data->socket_id);
      p_net_ctx->Connection[p_param_net_data->socket_id].SocketConnected = 1;
      break;

    case W61_NET_EVT_SOCK_DISCONNECTED_ID:
      LogDebug(" W6X got NET_EVT_SOCK_DISCONNECTED_ID for connection %d", p_param_net_data->socket_id);
...
static int32_t W6X_Net_Wait_Pull_Data(int32_t sock, int32_t connection_id, void *buf, size_t max_len)
{
  ...
  if ((p_net_ctx->Connection[connection_id].DataAvailable != NULL) &&
      (xSemaphoreTake(p_net_ctx->Connection[connection_id].DataAvailable,
                      (TickType_t) p_net_ctx->Sockets[sock].RecvTimeout) == pdTRUE))
  {
